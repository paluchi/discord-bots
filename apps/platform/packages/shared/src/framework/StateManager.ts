...
class StateManager {
  ...
  async setChatData(chatId: string, insights: Insights): Promise<void> {
  ...
  async getChatData(chatId: string): Promise<Insights | null> {
    const key = `chat:${chatId}`;
  ...
  async updateChatData(
    ...
    const updatedData = options?.overwrite
      ? insights
